...
from extensions.map_testing.testlog import TestLog, archive_testlog, TESTLOG_PAGE
...
from utils.conn import ddnet_delete, UploadFailedError
from utils.text import fmt_days, sanitize, to_discord_timestamp
...
class TestingManager:
    def __init__(self, bot):
        ...
        # set once load_testing_channels has hydrated test_channels
        self.channels_loaded = asyncio.Event()
    ...
    async def load_testing_channels(self):
        ...
        category_registry.invalidate(guild)
        categories = category_registry.all_categories(guild)
        channels = [
            channel
        ...
        log.info(
            "Loading %d testing channels from %d categories: %s",
            len(channels), len(categories), ", ".join(c.name for c in categories),
        )
    ...
    async def name_conflict(self, name: str) -> str | None:
        ...
        fetched and nothing is cached, so callers can fail closed rather than
    ...
    async def approve_submission(
        ...
        try:
            ...
            await self.report_initial_upload_failure(tc, exc)
        else:
    ...
    @staticmethod
    async def report_initial_upload_failure(tc: TestingChannel, exc: Exception) -> None:
        """Tell the channel its map isn't on the test servers yet"""
        reason = getattr(exc, "error", "The upload to ddnet.org failed.")
        try:
            await tc.channel.send(
                view=InitialUploadFailed(tc, reason),
                allowed_mentions=discord.AllowedMentions(users=True),
            )
        except discord.HTTPException as send_exc:
            log.warning("Couldn't post the upload failure notice in #%s: %s", tc.channel, send_exc)
    ...
    async def handle_testing_submission(self, message: discord.Message, tc: TestingChannel):
        ...
        if await self.same_as_current(tc, submission):
            await self.handle_identical_upload(tc, submission, message)
            return
        ...
        if is_trusted and filename_matches and not debug_output:
            ...
            try:
                ...
                log.error("Auto-upload failed in #%s: %s", tc.channel, error)
                await message.reply(
                    view=UploadFailed(error.error),
                    mention_author=False,
                    allowed_mentions=discord.AllowedMentions.none(),
                )
                return
            ...
            await self.write_changelog(
                tc, message.author,
                category="MapTesting/UPLOAD",
    ...
    async def handle_identical_upload(
            self, tc: TestingChannel, submission: Submission, message: discord.Message
    ) -> None:
        if tc.submission is not None and tc.submission.is_uploaded:
            await message.reply(
        ...
        try:
            await self.reupload_current(tc, submission)
        except UploadFailedError as error:
            log.error("Upload retry failed in #%s: %s", tc.channel, error)
            await message.reply(
                view=UploadFailed(error.error),
                mention_author=False,
                allowed_mentions=discord.AllowedMentions.none(),
    ...
    async def reupload_current(self, tc: TestingChannel, submission: Submission) -> None:
        await upload_submission(self.bot.session, submission, tc, self.bot.config)
        await self.set_current_map(tc, submission)
        await self.write_changelog(
            tc, submission.message.author,
            category="MapTesting/UPLOAD",
            string=f'The current version of "{tc.map_name}" was uploaded to the test servers.',
            url=submission.message.jump_url,
        )
        submission.bytes = None  # release the in-memory map; re-fetched lazily if needed
        log.info("Re-uploaded the current map in #%s after an earlier upload failure", tc.channel)
    ...
    async def confirm_upload(self, message: discord.Message, member) -> bool:
        ...
        Returns False (uploading nothing) when it is identical to a current map that
        is already live on the test backend."""
        ...
        if await self.same_as_current(tc, submission):
            if tc.submission is not None and tc.submission.is_uploaded:
                ...
                return False
            # current version never reached the backend, so this approval is a retry
            await self.reupload_current(tc, submission)
            return True
    ...
    async def same_as_current(tc: TestingChannel, submission: Submission) -> bool:
        """
        True if `submission` is byte-identical to the channel's current map. That
        alone doesn't mean there's nothing to do, see handle_identical_upload.
        """
    ...
    async def post_version_diff(self, tc: TestingChannel, previous: Submission | None, new: Submission) -> None:
        ...
        try:
            thread = await new.message.create_thread(name=DIFF_THREAD_NAME, auto_archive_duration=60)
        except discord.HTTPException as exc:
        ...
        try:
            ...
            log.warning("Couldn't archive the diff thread in #%s: %s", tc.channel, exc)

    async def write_changelog(
            self, tc: TestingChannel, actor, *, category: str, string: str, url: str | None = None
    ) -> None:
        try:
            ...
            log.exception("Changelog update failed in #%s", tc.channel)
    ...
    async def release_channel(
            self, tc: TestingChannel, actor, *, changelog_string: str, changelog_url: str | None = None
        ...
        await self.apply_transition(
            tc, transition, actor, changelog_string=changelog_string, changelog_url=changelog_url
        )

        grace_end = to_discord_timestamp(discord.utils.utcnow() + self.grace_period, style="F")
        return await tc.channel.send(
            view=MapReleased(tc, grace_end, fmt_days(self.grace_period)),
            allowed_mentions=discord.AllowedMentions(users=True),
        )
    ...
    async def handle_map_release(self, message: discord.Message) -> None:
        ...
        notice = await self.release_channel(
            ...
            changelog_url=message.jump_url,
        )
        if notice is not None:
    ...
    async def apply_author_update(self, tc: TestingChannel, uploader) -> None:
        ...
        READY drops to RC keeping one placeholder Tester vote
        WAITING returns to RC (if a Tester had approved) else TESTING
        """
        ...
        await self.apply_transition(
            ...
            changelog_url=tc.submission.message.jump_url if tc.submission else None,
    ...
    async def move_to_waiting_after_bugs(self, tc: TestingChannel, upload_message: discord.Message) -> None:
        ...
        await self.apply_transition(
            ...
            changelog_url=upload_message.jump_url,
    ...
    @property
    def delete_on_archive(self) -> bool:
    ...
    def grace_period(self) -> timedelta:
        """How long a released map channel stays open for final fixes before archiving"""
        return timedelta(
            days=self.bot.config.getfloat("TESTING_CHANNELS", "GRACE_PERIOD_DAYS", fallback=14)
        )

    def testlog_page(self, tc: TestingChannel) -> str | None:
        """Public link to the archived history"""
        if self.bot.config.getboolean("TESTING_CHANNELS", "TESTLOG_UPLOAD", fallback=False):
            return TESTLOG_PAGE + tc.filename
        return None

    async def archive_channel(self, tc: TestingChannel, *, reason: str) -> tuple[bool, str]:
        config = self.bot.config
        ...
        try:
            ...
            return False, "the testlog export crashed, see the bot logs"

        if not ok:
            log.error("Archive incomplete for #%s; channel kept.", tc.channel)
            return False, "some assets couldn't be saved or uploaded, see the bot logs"
        ...
        if not self.delete_on_archive:
            log.info("Archived #%s; channel kept (delete-on-archive off).", tc.channel)
            return True, summary + "\nChannel kept (`TESTLOG_DELETE_ON_ARCHIVE` is off)."

        await self.send_archive_notices(tc, reason)
        log.info("Archived #%s; deleting channel (%s).", tc.channel, reason)
        await tc.channel.delete(reason=f"Map testing channel archived: {reason}")
        return True, f"{summary} The channel was deleted."

    async def send_archive_notices(self, tc: TestingChannel, reason: str) -> None:
        testlog_url = self.testlog_page(tc)
        link_line = (
            f"\nThe full channel history stays available here: {testlog_url}"
            if testlog_url else ""
        )

        dm_text = (
                f"Your testing channel for **{tc.map_name}** has been archived "
                f"because {reason}." + link_line
        )
        for author in tc.authors:
            try:
                await author.send(view=ChannelArchived(dm_text))
            except discord.HTTPException as exc:
                log.info("Couldn't DM %s about the archive of %r: %s", author, tc.map_name, exc)

        tester_chat = self.bot.get_channel(Channels.TESTER_CHAT)
        if tester_chat is None:
            log.warning("TESTER_CHAT channel not found, skipping the archive notice")
            return
        notice_text = (
            f"**{tc.map_name}** has been archived because {reason}.{link_line}"
        )
        try:
            await tester_chat.send(
                view=ChannelArchived(notice_text),
                allowed_mentions=discord.AllowedMentions.none(),
            )
        except discord.HTTPException as exc:
            log.warning("Couldn't post the archive notice for %r: %s", tc.map_name, exc)
    ...
    async def handle_channel_delete(self, channel: discord.abc.GuildChannel) -> None:
        ...
        # clean up
        await changelog.delete_channel(channel.id)
    ...
    async def lookup_channel_deleter(channel: discord.abc.GuildChannel) -> str | None:
        ...
        return str(entry.user) if entry and entry.target.id == channel.id else None
