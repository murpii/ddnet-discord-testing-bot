...
class TestingListener(commands.Cog):
    ...
    async def cleanup_pin_messages(self, message: discord.Message):
        ...
        if (
                ...
                and category_registry.holds_map_channels(message.channel)
        ):
            with contextlib.suppress(discord.HTTPException):
    ...
    async def on_guild_channel_delete(self, channel: discord.abc.GuildChannel):
        if isinstance(channel, discord.CategoryChannel):
            category_registry.invalidate(channel.guild)
            return
        await self.bot.testing_manager.handle_channel_delete(channel)

    @commands.Cog.listener()
    async def on_guild_channel_create(self, channel: discord.abc.GuildChannel):
        # A new category may be an overflow one the registry should start using.
        if isinstance(channel, discord.CategoryChannel):
            category_registry.invalidate(channel.guild)
    ...
    async def on_guild_channel_update(
        ...
        # overflow categories are matched by name, so a rename can add or drop one
        if isinstance(after, discord.CategoryChannel) and before.name != after.name:
            category_registry.invalidate(after.guild)
