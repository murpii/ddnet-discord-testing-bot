...
async def build_channel(
    ...
    # First category of the state's group with room. A new one is added if all are full.
    ...
    log.info(
        "Created testing channel #%s (%d) in #%s", channel_name, discord_channel.id, category.name
    )
