...
class DiffPaginator(ErrorReporter, discord.ui.View):
    """
...
async def render_overlay(old_bytes: bytes, new_bytes: bytes, result):
    return await render_diff_images(new_bytes, result)


async def render_side_by_side(old_bytes: bytes, new_bytes: bytes, result):
...
async def render_and_send(interaction: discord.Interaction, old_id: int, new_id: int, *, kind: str, renderer):
    ...
    old_sub = await map_submission(source, old_id)
    new_sub = await map_submission(source, new_id)
    ...
    cached = load_cached_diff(channel_id, key, kind=kind)
    if cached is None:
        result = await MapDiff.diff(old_sub, new_sub)
        images, total = await renderer(old_bytes, new_bytes, result)
        if not images:
            await interaction.followup.send("Couldn't render the map image(s).", ephemeral=True)
            return
        store_cached_diff(channel_id, key, images, total, kind=kind)
        cached = (images, total)
...
class VisualDiffButton(
    ...
    async def callback(self, interaction: discord.Interaction):
        await send_diff(interaction, self.old_id, self.new_id, kind="", renderer=render_overlay)


class SideBySideDiffButton(
    discord.ui.DynamicItem[discord.ui.Button],
    template=r"mt_sxs:(?P<old>\d+):(?P<new>\d+)",
):
    def __init__(self, old_id: int, new_id: int):
        self.old_id = old_id
        self.new_id = new_id
        super().__init__(discord.ui.Button(
            label="Before / After",
            style=discord.ButtonStyle.secondary,
            custom_id=f"mt_sxs:{old_id}:{new_id}",
        ))
    ...
    async def callback(self, interaction: discord.Interaction):
        await send_diff(interaction, self.old_id, self.new_id, kind="sxs", renderer=render_side_by_side)


class VersionDiff(ErrorReporter, discord.ui.LayoutView):
    """
    ...
    def __init__(
        ...
        if show_visual_diff:
            ...
            row.add_item(SideBySideDiffButton(old_id, new_id))
            self.add_item(row)
