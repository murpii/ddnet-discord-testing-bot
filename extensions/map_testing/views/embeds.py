...
class MapReleased(discord.ui.LayoutView):
    def __init__(self, tc, timestamp, grace_duration: str):
        super().__init__(timeout=None)
        self.add_item(discord.ui.Container(
            discord.ui.TextDisplay(
                "# 🎉 Map Released!\n"
                f"{tc.mapper_mentions} your map has just been released!\n\n"
                f"You now have a grace period of **{grace_duration}** to identify and resolve any unnoticed bugs or skips. "
...
class GraceEndingSoon(discord.ui.LayoutView):
    def __init__(self, tc, ends: str):
        super().__init__(timeout=None)
        self.add_item(discord.ui.Container(
            discord.ui.TextDisplay(
                f"{tc.mapper_mentions} The grace period for **{tc.map_name}** ends {ends}. "
                "If you have any final fixes, submit them before the channel gets archived."
            ),
            accent_colour=discord.Color.orange(),
        ))


class GraceOver(discord.ui.LayoutView):
    def __init__(self, tc):
        super().__init__(timeout=None)
        self.add_item(discord.ui.Container(
            discord.ui.TextDisplay(
                f"The grace period for **{tc.map_name}** has ended. "
                "The channel will be archived automatically in a few days."
            ),
            accent_colour=discord.Color.darker_grey(),
        ))


class ChannelArchived(discord.ui.LayoutView):
...
class UploadFailed(discord.ui.LayoutView):
    def __init__(self, reason: str):
        super().__init__(timeout=None)
        self.add_item(discord.ui.Container(
            discord.ui.TextDisplay(
                "## ⚠️ Upload failed\n"
                "The current map version was left unchanged.\n"
                f"-# {reason}\n"
...
class InitialUploadFailed(discord.ui.LayoutView):
    def __init__(self, tc, reason: str):
        super().__init__(timeout=None)
        self.add_item(discord.ui.Container(
            discord.ui.TextDisplay(
                "## ⚠️ Upload failed\n"
                f"{tc.mapper_mentions} Your map is not on the test servers yet.\n"
                f"-# {reason}\n"
                "-# Post the map file here again to retry the upload."
            ),
            accent_colour=discord.Color.red(),
        ))


class UploadRetried(discord.ui.LayoutView):
    def __init__(self):
        super().__init__(timeout=None)
        self.add_item(discord.ui.Container(
            discord.ui.TextDisplay(
                "## 🆙 Uploaded\n"
                "Same map as the current version, but that one never reached the "
                "test servers. It's uploaded now."
            ),
            accent_colour=discord.Color.green(),
        ))
