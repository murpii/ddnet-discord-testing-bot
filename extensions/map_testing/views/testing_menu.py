...
class TestingMenu(ErrorReporter, discord.ui.LayoutView):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        # Channel-edit cooldown: ~2 edits / 15 min per channel
    ...
    async def mt_archive(self, interaction: discord.Interaction):
        await interaction.response.defer(thinking=True, ephemeral=True)
        tc = self.bot.testing_manager.get_tc_from_interaction(interaction)
        ok, detail = await self.bot.testing_manager.archive_channel(
            tc, reason="a tester archived it manually"
        )
        if ok:
            message = detail
        else:
            message = f"Archiving failed, {detail}. The channel was kept."

        # the followup can 404 when the archive just deleted this channel
