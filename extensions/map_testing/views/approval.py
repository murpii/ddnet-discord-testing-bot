...
from extensions.map_testing.categories import NoCategorySpace
...
from utils.conn import UploadFailedError
...
class ReferencedView(ErrorReporter, discord.ui.LayoutView):
    """
    ...
    async def gone(interaction: discord.Interaction) -> None:
        ...
        if ref is not None and ref.message_id is not None:
            interaction.client.testing_manager.untrack_approval(ref.message_id) # noqa
...
class SubmitCleanApproval(ReferencedView):
    ...
    async def approve(self, interaction: discord.Interaction) -> None:
        ...
        except NoCategorySpace as exc:
            log.error("No room for a new testing channel: %s", exc)
        ...
        if result is ApprovalResult.CREATED:
            ...
            await interaction.message.edit(
                view=resolved(
                    f"❌ Declined by {interaction.user.mention}. The map name is already taken.",
...
class SubmitBuggyApproval(ReferencedView):
    ...
    def __init__(self, bot):
        ...
        self.add(
            "## ⚠️ Map Bugs found!\n"
            "Your submission didn't pass the automatic checks, check **View Debug Output** for the details.\n"
...
class ChannelUploadApproval(ReferencedView):
    """
    In-channel: a pending upload held for manual approval.
    Approve uploads it as-is and makes it the current map.
    """
    ...
    async def approve_upload(self, interaction: discord.Interaction):
        ...
        try:
            ...
            log.warning("Upload approval failed for message %s: %s", submission.id, error)
            await interaction.followup.send(
                f"Upload failed. {error.error} Please try again in a moment.", ephemeral=True
            )
            return
        except Exception:  # noqa
